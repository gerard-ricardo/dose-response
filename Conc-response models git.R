...
##################################################################################
# 6) Non-linear regression ( 4-par Weibull)
library(drc) #NLR
...
#     NEC~dnorm(5, 0.0001) T(0,)   #mean set to 30, contrained to >0
#     
#     # likelihood
#     for (i in 1:N)
#     {
...
# inits <- function(){list(top = 0.9, beta = 0.2, NEC = 5)}  #you will need to change this
# 
# library(R2jags)
...
ec1 = ECfun(md1,inh10.1)  #binom GLM
ec2 = ECfun(md2,inh10.2 )  #quasi GLM
ec3 = ECfun2(md3,inh10.3 )  #binom GLMM
ec4.b = ED(md4, ec, type = c("relative"), interval = "delta")  #4-par-logisitc
ec4 = data.frame(ecx = ec4.b[1], lower = ec4.b[3], upper = ec4.b[4])
ec5.b = ED(md5, ec, type = c("relative"), interval = "delta")
ec5 = data.frame(ecx = ec5.b[1], lower = ec5.b[3], upper = ec5.b[4])  #Weighted binomial 4-par logistic
ec6.b = ED(md6, ec, type = c("relative"), interval = "delta")
ec6 = data.frame(ecx = ec6.b[1], lower = ec6.b[3], upper = ec6.b[4])  # 4-par Weibull
ec7 = ECfun2(md7,inh10.7 )  #bayes GLM - wealky informative prior
ec8 = ECfun2(md8,inh10.8 )  #bayes GLMM - wealky informative prior
...
NEC.bayes = c(out$NEC[2], out$NEC[1], out$NEC[3])
NEC.ec10 = extract_ECx(out, ECx.val = 10)
NEC.drc = c(coef(nec.m1)[4], confint(nec.m1)[4], confint(nec.m1)[8])

#thresholds 

ec.all =rbind(ec1, ec2, ec3, ec4, ec5, ec6, ec7, ec8, NEC.bayes, NEC.ec10, NEC.drc)
#row.names(ec.all) <- c("Binom GLM", 'Quasi GLM', "GLMM", 'LL.4', 'Binom LL.4', 'Weibull','Bayesian GLM','Bayesian GLMM', 'Nec.bayes', NEC.ec10, 'NEC.drc')
model <- c("Binom GLM", 'Quasi GLM', "GLMM", 'LL.4 NLR', 'Binom LL.4 NLR', 'Weibull NLR','Bayesian GLM','Bayesian GLMM', 'Nec.bayes', 'NEC.ec10', 'NEC.drc')
ec.all$model = model
ec.all$model <- factor(ec.all$model,levels = c("Binom GLM", "Quasi GLM", "GLMM", "LL.4 NLR", "Binom LL.4 NLR",'Weibull NLR','Bayesian GLM','Bayesian GLMM', 'Nec.bayes', 'NEC.ec10', 'NEC.drc')) #Set levels in order
ec.all
...
p = p + theme_bw()
p = p + scale_y_continuous(name ="Thresholds")
p = p + scale_x_discrete(name ="Models")
p = p + coord_flip()
